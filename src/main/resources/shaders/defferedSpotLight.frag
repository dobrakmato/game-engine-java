...
void main() {
    ...
    // Sample gbuffer.
    vec3 v0 = texture(c0, texCoord).rgb;
    ...
    if (castingShadows) {
        ...
        vec3 scPostW = shadowCoord0.xyz / shadowCoord0.w;
        scPostW = scPostW * 0.5 + 0.5; // replace with bias mat

        bool outsideShadowMap = shadowCoord0.w <= 0.0f || (scPostW.x < 0 || scPostW.y < 0) || (scPostW.x >= 1 || scPostW.y >= 1);
        if (!outsideShadowMap) {
     	    shadowFactor = min(1.0, shadow_smooth2(shadowMap, scPostW));
        }
    }
    ...
    if (spotFactor + 0.0 > light.cutoff + 0.0) {
        ...
        gl_FragColor.rgb = (diffuse * lambert * light.color * light.intensity);
        vec3 c = shadingSpecularGGX(normal, viewDir, lightDir, roughness, specular);

        gl_FragColor.rgb += c * light.intensity * light.color;
        gl_FragColor.rgb *= attenuation;
        gl_FragColor.rgb *= shadowFactor;
        gl_FragColor.rgb *= (1 - (1 - spotFactor) / (1 - light.cutoff));
    }
